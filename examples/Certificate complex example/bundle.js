(function (exports) {
	...
	class RSAPrivateKey
	{
		...
		fromJSON(json)
		{
			...
			if("d" in json)
				this.privateExponent = new Integer({ valueHex: stringToArrayBuffer(fromBase64(json.d, true)).slice(0, 256) });
			else
				throw new Error("Absent mandatory parameter \"d\"");

			if("p" in json)
				this.prime1 = new Integer({ valueHex: stringToArrayBuffer(fromBase64(json.p, true)).slice(0, 128) });
			else
				throw new Error("Absent mandatory parameter \"p\"");

			if("q" in json)
				this.prime2 = new Integer({ valueHex: stringToArrayBuffer(fromBase64(json.q, true)).slice(0, 128) });
			else
				throw new Error("Absent mandatory parameter \"q\"");

			if("dp" in json)
				this.exponent1 = new Integer({ valueHex: stringToArrayBuffer(fromBase64(json.dp, true)).slice(0, 128) });
			else
				throw new Error("Absent mandatory parameter \"dp\"");

			if("dq" in json)
				this.exponent2 = new Integer({ valueHex: stringToArrayBuffer(fromBase64(json.dq, true)).slice(0, 128) });
			else
				throw new Error("Absent mandatory parameter \"dq\"");

			if("qi" in json)
				this.coefficient = new Integer({ valueHex: stringToArrayBuffer(fromBase64(json.qi, true)).slice(0, 128) });
			else
	 ...
	 */
	function concatBuffers(inputBuf1, inputBuf2)
	{
	 ...
	 */
	function toHexCodes(inputBuffer, inputOffset, inputLength)
	{
	 ...
	 * @returns {string}
	 */
	function fromBase64(input, useUrlTemplate = false)
	{
		const template = (useUrlTemplate) ? base64UrlTemplate : base64Template;
	...
	class LocalConstructedValueBlock extends LocalValueBlock
	{
		...
		fromBER(inputBuffer, inputOffset, inputLength)
		{
			...
			while(checkLen(this.isIndefiniteForm, inputLength) > 0)
			{
				...
				if((this.isIndefiniteForm === true) && (returnObject.result.blockName() === EndOfContent.blockName()))
					break;
			}

			if(this.isIndefiniteForm === true)
			{
				if(this.value[this.value.length - 1].blockName() === EndOfContent.blockName())
					this.value.pop();
				else
	...
	class LocalOctetStringValueBlock extends LocalHexBlock(LocalConstructedValueBlock)
	{
		...
		fromBER(inputBuffer, inputOffset, inputLength)
		{
			...
			if(this.isConstructed === true)
			{
				...
				for(let i = 0; i < this.value.length; i++)
				{
					const currentBlockName = this.value[i].blockName();
	...
	class Integer extends BaseBlock
	{
		 ...
		 * @returns {Integer}
		 */
		convertFromDER(expectedLength = 0)
		{
			const integer = new Integer({ valueHex: this.valueBlock.valueHex });
	...
	class LocalSidValueBlock extends LocalHexBlock(LocalBaseBlock)
	{
		...
		toString()
		{
			...
			if(this.isHexOnly === true)
				result = toHexCodes(this.valueHex, 0, this.valueHex.byteLength);
			else
			{
				if(this.isFirstSid)
				{
	...
	class Repeated
	{
		...
		constructor(parameters = {})
		{
			this.name = getParametersValue(parameters, "value", "");
			this.optional = getParametersValue(parameters, "optional", false);
	...
	class Time$1
	{
		...
		constructor(parameters = {})
		{
			 ...
			 * @description 0 - UTCTime; 1 - GeneralizedTime; 2 - empty value
			 */
			this.type = getParametersValue(parameters, "type", Time$1.defaultValues("type"));
			/**
			 * @type {Date}
			 * @description Value of the TIME class
			 */
			this.value = getParametersValue(parameters, "value", Time$1.defaultValues("value"));
			//endregion
		...
		{
			//region Check the schema is valid
			const asn1 = compareSchema(schema, schema, Time$1.schema({
				names: {
		...
		return (new Sequence({
			...
			value: [
				...
				new Sequence({
					...
					value: [
						Time$1.schema(names.notBefore || {
							names: {
								utcTimeName: "tbsCertificate.notBefore",
								generalTimeName: "tbsCertificate.notBefore"
							}
						}),
						Time$1.schema(names.notAfter || {
							names: {
	...
	class Certificate
	{
		...
		fromSchema(schema)
		{
			...
			this.notBefore = new Time$1({ schema: asn1.result["tbsCertificate.notBefore"] });
			this.notAfter = new Time$1({ schema: asn1.result["tbsCertificate.notAfter"] });
	...
	class RevokedCertificate
	{
		...
		fromSchema(schema)
		{
			...
			this.revocationDate = new Time$1({ schema: asn1.result.revocationDate });
	...
	function tbsCertList(parameters = {})
	{
		...
		return (new Sequence({
			...
			value: [
				...
				Time$1.schema(names.tbsCertListThisUpdate || {
					names: {
						utcTimeName: "tbsCertList.thisUpdate",
						generalTimeName: "tbsCertList.thisUpdate"
					}
				}),
				Time$1.schema(names.tbsCertListNextUpdate || {
					names: {
	...
	class CertificateRevocationList
	{
		...
		fromSchema(schema)
		{
			...
			this.thisUpdate = new Time$1({ schema: asn1.result["tbsCertList.thisUpdate"] });
			if("tbsCertList.nextUpdate" in asn1.result)
				this.nextUpdate = new Time$1({ schema: asn1.result["tbsCertList.nextUpdate"] });
	...
	{
		if(engine$1.subtle !== null)
			return engine$1.subtle;
	 ...
	 * @returns {*}
	 */
	function getRandomValues$1(view)
	{
		if(engine$1.crypto !== null)
			return engine$1.crypto.getRandomValues(view);
	 ...
	 * @returns {*}
	 */
	function getAlgorithmParameters$1(algorithmName, operation)
	{
		let result = {
			algorithm: {},
			usages: []
				...
				switch(operation.toLowerCase())
				{
					...
					case "encrypt":
						result = {
							algorithm: {
								name: "AES-CBC",
								iv: getRandomValues$1(new Uint8Array(16)) // For "decrypt" the value should be replaced with value got on "encrypt" step
							},
							usages: ["encrypt", "decrypt", "wrapKey", "unwrapKey"]
						};
						break;
	 ...
	 * @param {!number} inputLength Maximum length of array of bytes which can be using in this function
	 */
	function toHexCodes$1(inputBuffer, inputOffset, inputLength)
	{
		//region Check input buffer parameters
		if((inputBuffer instanceof ArrayBuffer) === false)
			return "";
	...
	function parseCertificate()
	{
		...
		document.getElementById("cert-serial-number").innerHTML = toHexCodes$1(certificate.serialNumber.valueBlock.valueHex);
		//endregion
	...
	//*********************************************************************************
	function createCertificate(buffer)
	{
		//region Initial variables 
		let sequence = Promise.resolve();
		...
		sequence = sequence.then(() => {
			...
			const algorithm = getAlgorithmParameters$1(signatureAlgorithmName, "generatekey");
			if("hash" in algorithm.algorithm)
	...
	function verifyCertificate()
	{
		...
		// #region Verify CERT
		certChainVerificationEngine.verify().
		then(result => {
			...
			if(started === true)
			{
				...
				else
				{
					if(String.fromCharCode(view[i]) === "-")
					{
						...
						const asn1 = fromBER(stringToArrayBuffer$1(window.atob(certBodyEncoded)));
						try
						{
	...
	exports.parseCAbundle = parseCAbundle;
	exports.handleFileBrowse = handleFileBrowse;
	exports.handleTrustedCertsFile = handleTrustedCertsFile;
	exports.handleInterCertsFile = handleInterCertsFile;
	exports.handleCRLsFile = handleCRLsFile;
	exports.handleCABundle = handleCABundle;
